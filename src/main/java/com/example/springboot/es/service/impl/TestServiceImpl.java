package com.example.springboot.es.service.impl;

import com.example.springboot.es.bean.TestBean;
import com.example.springboot.es.dao.TestDao;
...
//                .withQuery(QueryBuilders.matchQuery("desc", text))
                .withQuery(QueryBuilders.multiMatchQuery( text,"desc","name"))
        ...
        SearchHits<TestBean> search = template.search(query, TestBean.class,IndexCoordinates.of("testdoct"));
        long totalHits = search.getTotalHits();
        log.info("totalHits={}",totalHits);
        List<TestBean> list = new ArrayList<>();
        search.forEach(searchHit->
        {
            TestBean content = searchHit.getContent();
            Map<String, List<String>> highlightFields = searchHit.getHighlightFields();
            if (highlightFields != null) {
                if (!CollectionUtils.isEmpty(highlightFields.get("desc"))) {
                    String desc = highlightFields.get("desc").get(0);
                    content.setDesc(desc);
                }
                if (!CollectionUtils.isEmpty(highlightFields.get("name"))) {
                    String name = highlightFields.get("name").get(0);
